...
function GameCanvas() {
    ...
    useEffect(() => {
        ...
        const initGame = () => {
            ...
            k.loadRoot("/assets/animation_frames");

            // Load all assets
            const v = Date.now();
            k.loadSprite("b-cell-neutral", `B-Cells/B-Cell_Idle(Neutral Form).png?v=${v}`);
            k.loadSprite("b-cell-squash", `B-Cells/B-Cell_Idle(Squash Form).png?v=${v}`);
            k.loadSprite("b-cell-stretch", `B-Cells/B-Cell_Idle(Stretch Form).png?v=${v}`);
            k.loadSprite("flu-virus", `Flu Virus/Flu-virus.png?v=${v}`);
            k.loadSprite("flu-virus-death", `Flu Virus/Flu-virus_Death.png?v=${v}`);
            k.loadSprite("y-antibody", `B-Cells/Y-Antibody_projectile.png?v=${v}`);
            k.loadSprite("macrophage-idle-neutral", `Macrophage/Macrophage_Idle(Neutral).png?v=${v}`);
            k.loadSprite("macrophage-idle-excited", `Macrophage/Macrophage_Idle(Excited).png?v=${v}`);
            k.loadSprite("macrophage-prepare", `Macrophage/Macrophage_Attack(Prepare_To_Eat).png?v=${v}`);
            k.loadSprite("macrophage-attack", `Macrophage/Macrophage_Attack(Big_Munch).png?v=${v}`);
            k.loadSprite("platelet-idle", `Platelet/Platelet_Idle.png?v=${v}`);
            k.loadSprite("platelet-idle2", `Platelet/Platelet_Idle1.png?v=${v}`);
            k.loadSprite("platelet-prepare", `Platelet/Platelet_PrepareToThrow.png?v=${v}`);
            k.loadSprite("platelet-throw", `Platelet/Platelet_AfterThrowSwing.png?v=${v}`);
            k.loadSprite("fibrin-projectile", `Platelet/Fibrin-net_Projectile.png?v=${v}`);
            k.loadSprite("fibrin-expanded", `Platelet/Fibrin-net_Expanded.png?v=${v}`);
            k.loadSprite("basophil-idle", `Basophil/Basophil_Idle.png?v=${v}`);
            k.loadSprite("basophil-idle2", `Basophil/Basophil_Idle1.png?v=${v}`);
            k.loadSprite("basophil-throw", `Basophil/Basophil_Throw.png?v=${v}`);
            k.loadSprite("bomb-projectile", `Basophil/Histamin_Bomb_Projectile.png?v=${v}`);
            k.loadSprite("explosion-effect", `Basophil/Explosion_Effect.png?v=${v}`);
